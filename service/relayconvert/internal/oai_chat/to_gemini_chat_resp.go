...
func StreamResponseOpenAI2Gemini(openAIResponse *dto.ChatCompletionsStreamResponse) *dto.GeminiChatResponse {
	// 检查是否有实际内容或结束标志
	hasContent := false
	...
	for _, choice := range openAIResponse.Choices {
		...
		// 设置结束原因 (ChatCompletionsStreamResponseChoice.FinishReason 是 *string)
		if choice.FinishReason != nil {
			var finishReason string
			switch *choice.FinishReason {
			case "stop":
