...
function getUsageLogDetailSummary(record, text, t) {
  ...
  const segments = [];
  if (other?.upstream_model_name && other.upstream_model_name !== record.model_name) {
    segments.push({ text: `${t('实际模型')}: ${other.upstream_model_name}`, tone: 'muted' });
  }
  if (other?.admin_info?.local_count_tokens) {
    segments.push({ text: t('本地统计 Tokens'), tone: 'muted' });
  }
  if (Array.isArray(other?.po) && other.po.length > 0) {
    segments.push({ text: `${t('参数覆盖')}: ${other.po.length}`, tone: 'warning' });
  }
...
function getTotalTokens(record) {
  ...
  return [
    ...
    },
    {
      ...
      render: (text, record, index) => {
        ...
        return <>{formatTokenCount(getTotalTokens(record))}</>;
      },
    },
    {
      key: COLUMN_KEYS.IP,
