...
Root 管理员可在「系统设置 → 速率限制」中配置全局 API 速率限制，包括启用开关、每个周期允许的请求次数和周期秒数。该限制默认关闭；启用后按请求来源执行全局 API 访问限制。模型请求限流、全局 Web 限流和关键接口限流仍分别独立配置，不会因调整全局 API 限流而改变。

配置保存后立即更新运行时设置，无需重启服务。请求次数必须为 `1` 至 `100000000`，限制周期必须为 `1` 至 `1200` 秒；具体数值应根据部署规模和上游服务承载能力谨慎设置。
