...
type BodyStorage interface {
	...
	// IsDisk 是否是磁盘存储
	IsDisk() bool
	// NewReader returns an independent reader positioned at the beginning of
	// the stored payload. Closing the returned reader never closes the storage.
...
func (m *memoryStorage) NewReader() (io.ReadCloser, error) {
	...
	// bytes.Reader has an independent cursor and reuses the immutable backing
	// slice, so retry readers require no additional payload allocation.
...
func (d *diskStorage) NewReader() (io.ReadCloser, error) {
	...
	// Use a separate descriptor and cursor so request retries cannot disturb
	// the root storage reader or extend its ownership lifecycle.
	file, err := os.Open(d.filePath)
	if err != nil {
		return nil, fmt.Errorf("open body cache file for replay: %w", err)
	}
	return file, nil
}
