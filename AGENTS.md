...
### 规则 8.1：经授权的第三方渠道签到任务例外

在管理员明确授权、且仅针对其拥有或有权管理的第三方渠道时，可以实现受限的签到任务管理与定时执行。该例外仅用于发送预先配置的签到请求和记录执行状态，不构成本地额度、计费、充值或第三方额度管理功能。

**允许范围：**
- 管理员提供的 HTTPS 请求地址、系统访问令牌或 Cookie、用户 ID，以及经明确配置的单一固定出站代理。
- 有上限的网络瞬态错误重试、请求超时、TLS 校验、脱敏执行日志、手动执行和“需人工处理”状态。
- 当响应为成功、已签到、认证失效、配置错误、网络错误、验证码页或风控/拦截页时进行明确分类；后两类必须停止自动执行并提示管理员在站点侧处理。

**严格禁止：**
- 验证码、WAF、反爬、Cloudflare 或其他访问控制的绕过、破解或规避。
- 代理池、代理轮换、自动代理切换、地域/指纹伪装，或任何规避站点访问限制的网络策略。
- 账号密码/OAuth 自动登录、CDK/充值、第三方额度读取、存储、计算或本地额度同步。
- 对 401、403、验证码、风控或 HTML 拦截页进行规避性重试。
