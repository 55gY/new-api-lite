...
export const useTokensData = (openFluentNotification, openCCSwitchModal) => {
  ...
  const syncPageData = (payload) => {
    ...
    setTokens(Array.isArray(pageData.items) ? pageData.items : []);
    setTokenCount(pageData.total || 0);
    setActivePage(pageData.page || 1);
    setPageSize(pageData.page_size || pageSize);
    setShowKeys({});
  };
  ...
  const rowSelection = {
    ...
    onChange: (selectedRowKeys, selectedRows) => {
      setSelectedKeys(Array.isArray(selectedRows) ? selectedRows : []);
    },
  };
  ...
  const batchDeleteTokens = async () => {
    const selectedTokens = Array.isArray(selectedKeys) ? selectedKeys : [];
    ...
    setLoading(true);
    try {
      const ids = selectedTokens.map((token) => token.id);
