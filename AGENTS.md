...
- 后续创建 Git 提交时，提交说明（subject 及需要时的 body）必须使用**简体中文**，准确概述实际变更。
- 提交说明应保持简洁、可检索，推荐使用“类型：变更摘要”的格式，例如：`功能：新增配置备份与还原`、`修复：调整多架构镜像选择`。
- 不得使用与实际变更不符的笼统说明；涉及破坏性变更时，必须在提交说明中明确标注。
