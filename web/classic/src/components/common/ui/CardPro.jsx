...
const CardScrollbarPortal = ({ containerRef, children }) => {
  ...
  useLayoutEffect(() => {
    ...
    const ensureHost = () => {
      ...
      const card = container?.querySelector('.table-scroll-card');
      const body = card?.querySelector(':scope > .semi-card-body');
      if (!card || !body) return false;

      const footer = card.querySelector(':scope > .semi-card-footer');
      if (!host) {
        host = document.createElement('div');
        host.className = 'floating-table-scrollbar-area';
        setTarget(host);
      }

      if (footer && footer.previousElementSibling !== host) {
        card.insertBefore(host, footer);
      } else if (!footer && body.nextElementSibling !== host) {
    ...
    const scheduleEnsureHost = () => {
      ...
      frameId = requestAnimationFrame(ensureHost);
    };
  ...
  const portalChildren = React.isValidElement(children)
    ? React.cloneElement(children, { useParentArea: true })
    : children;

  return createPortal(portalChildren, target);
};

CardScrollbarPortal.propTypes = {
...
const CardPro = ({
  ...
  return (
    <div ref={containerRef} className='table-scroll-card-wrapper'>
      <Card
      ...
      {tableScrollbarArea && (
        <CardScrollbarPortal containerRef={containerRef}>
          {tableScrollbarArea}
        </CardScrollbarPortal>
      )}
    </div>
...
CardPro.propTypes = {
  ...
  tableScrollbarArea: PropTypes.node,
  // 表格内容
  children: PropTypes.node,
  // 国际化函数
  t: PropTypes.func,
