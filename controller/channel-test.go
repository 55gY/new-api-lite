...
import (
	...
	"github.com/55gY/new-api-lite/relay"
...
func testChannel(channel *model.Channel, testUserID int, testModel string, endpointType string, isStream bool) (result testResult) {
	tik := time.Now()
	testedModelName := strings.TrimSpace(testModel)
	defer func() {
		if result.testedModel == "" {
			result.testedModel = testedModelName
		}
	}()
	...
	testedModelName = originModelName
	...
	other := buildTestLogOther(c, info, usage)
	model.RecordUsageLog(c, testUserID, model.RecordUsageLogParams{
	...
	return testResult{
		...
		localErr:     nil,
		newAPIError:  nil,
		testedModel:  originModelName,
		responseText: summarizeTestResponseBody(respBody),
	}
}

func buildTestLogOther(c *gin.Context, info *relaycommon.RelayInfo, usage *dto.Usage) map[string]interface{} {
	return service.GenerateTextOtherInfo(c, info, usage.PromptTokensDetails.CachedTokens)
}
...
func summarizeTestResponseBody(respBody []byte) string {
	const maxResponseTextRunes = 4096
	text := strings.TrimSpace(string(respBody))
	if text == "" {
		return ""
	}
	runes := []rune(text)
	if len(runes) <= maxResponseTextRunes {
		return text
	}
	return string(runes[:maxResponseTextRunes])
}
...
func autoDisableChannelIfAllModelsUnavailable(channel *model.Channel, testedModel string, testError string) {
	...
	if _, err := model.AutoDisableChannelIfAllModelsUnavailable(channel.Id, reason); err != nil {
		common.SysError(fmt.Sprintf("auto disable channel by model availability failed: channel_id=%d, error=%v", channel.Id, err))
	}
}
	...
	gopool.Go(func() {
		...
		for _, channel := range channels {
			...
			if len(testableModels) == 0 {
				allAbilities, err := model.GetChannelAbilities(channel.Id)
				if err != nil {
					common.SysError(fmt.Sprintf("get channel abilities failed: channel_id=%d, error=%v", channel.Id, err))
					continue
				}
			...
			if lastUnavailableReason != "" {
				autoDisableChannelIfAllModelsUnavailable(channel, channel.Name, lastUnavailableReason)
			}
		}
