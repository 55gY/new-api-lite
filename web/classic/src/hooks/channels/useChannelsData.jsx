...
export const useChannelsData = () => {
  ...
  useEffect(() => {
    const localIdSort = localStorage.getItem('id-sort') !== 'false';
    const localPageSize =
      parseInt(localStorage.getItem('page-size')) || ITEMS_PER_PAGE;
    const localEnableBatchDelete =
      localStorage.getItem('enable-batch-delete') !== 'false';

    setIdSort(localIdSort);
    setPageSize(localPageSize);
    setEnableBatchDelete(localEnableBatchDelete);

    loadChannels(1, localPageSize, localIdSort)
      .then()
      .catch((reason) => {
        showError(reason);
      });
  ...
  const loadChannels = async (
    ...
    if (searchKeyword !== '' || searchModel !== '') {
      setLoading(true);
      await searchChannels(typeKey, statusF, page, pageSize, idSort);
      setLoading(false);
      return;
    }
    ...
    const res = await API.get(
      `/api/channel/?p=${page}&page_size=${pageSize}&id_sort=${idSort}${typeParam}${statusParam}`,
    );
  ...
  const searchChannels = async (
    ...
    try {
      if (searchKeyword === '' && searchModel === '') {
        await loadChannels(page, pageSz, sortFlag, typeKey, statusF);
        return;
      }
      ...
      const statusParam = statusF !== 'all' ? `&status=${statusF}` : '';
      const res = await API.get(
        `/api/channel/search?keyword=${searchKeyword}&model=${searchModel}&id_sort=${sortFlag}&p=${page}&page_size=${pageSz}${typeParam}${statusParam}`,
      );
      const { success, message, data } = res.data;
      if (success) {
  ...
  const refresh = async (page = activePage) => {
    const { searchKeyword, searchModel } = getFormValues();
    if (searchKeyword === '' && searchModel === '') {
      await loadChannels(page, pageSize, idSort);
    } else {
      await searchChannels(activeTypeKey, statusFilter, page, pageSize, idSort);
    }
  };
  ...
  const handlePageChange = (page) => {
    ...
    setActivePage(page);
    if (searchKeyword === '' && searchModel === '') {
      loadChannels(page, pageSize, idSort).then(() => {});
    } else {
      searchChannels(activeTypeKey, statusFilter, page, pageSize, idSort);
    }
  };
  ...
  const handlePageSizeChange = async (size) => {
    ...
    const { searchKeyword, searchModel } = getFormValues();
    if (searchKeyword === '' && searchModel === '') {
      loadChannels(1, size, idSort)
        .then()
        .catch((reason) => {
          showError(reason);
        });
    } else {
      searchChannels(activeTypeKey, statusFilter, 1, size, idSort);
    }
  };
        ...
        if (!model || model === '') {
          showInfo(
            `${t('通道 ${name} 测试成功，耗时 ${time.toFixed(2)} 秒。')
              ...
              .replace('${time.toFixed(2)}', time.toFixed(2))}${response ? `\n${t('返回信息')}：${response}` : ''}`,
          );
        } else {
          showInfo(
            `${t(
